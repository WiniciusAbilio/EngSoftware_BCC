...
import jwt  # Certifique-se de que a biblioteca JWT está instalada

def middlewareAcessoAdm(get_response):
    def middleware(request):
        token = request.POST.get('token')  # Obtém o token JWT da solicitação
        ...
        try:
            ...
            if decoded_token.get('usuario_tipo') == 'admin':
                ...
                return JsonResponse({"error": "Acesso não autorizado para administradores"}, status=403)
